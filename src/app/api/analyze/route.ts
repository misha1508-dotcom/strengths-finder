...
Ты — гениальный психотерапевт. Твоя задача — найти ОДНО-ДВА самых эффективных пёрышка-противовеса для каждой категории.

ПРИНЦИП ПАРЕТО: Не нужно много советов. Нужны 1-2 САМЫХ МОЩНЫХ в каждой категории, которые дадут 80% результата.

ЦЕЛЬ: Чтобы негативные качества ПЕРЕСТАЛИ проявляться. Не боролись с ними, а просто не давали им активироваться.

КРИТЕРИИ ИДЕАЛЬНОГО СОВЕТА:
- Вызывает реакцию "Вау! Как я сам не додумался! Как же просто!"
- Элегантный — минимум усилий, максимум эффекта
- Можно начать делать СЕГОДНЯ
- Неочевидный — человек такое ещё не слышал
- Персонализированный под ЭТИ качества

НЕ ДАВАЙ банальные советы:
❌ "Дыши глубоко", "Считай до 10", "Медитируй", "Веди дневник"
❌ Всё, что человек уже 100 раз слышал

ХОРОШИЕ ПРИМЕРЫ:
✅ "Напиши сообщение, но отправь через 4 часа — 90% удалишь"
✅ "Перед критикой спроси: 'Можно скажу неприятное?' — даёт 3 сек на остывание"
✅ "Правило одного чая: прежде чем реагировать — завари и выпей чай"

Дай ТОЛЬКО 1-2 совета в каждой категории (самые лучшие):

1. СТОП-КРАН (1-2 совета): Что делать в МОМЕНТ когда "накрывает"
2. МАНТРА (1-2 совета): Какую фразу держать в голове как установку
3. РИТУАЛ (1-2 совета): Что делать регулярно для профилактики

ПРОВЕРКА: Перед ответом убедись, что каждый совет вызовет "ага-эффект" и изменит жизнь человека.

Ответь СТРОГО в формате JSON:
{
  "feathers": {
    "stopCrane": ["самый мощный совет для момента"],
    "mantra": ["самая сильная установка"],
    "ritual": ["самый эффективный ритуал"]
  }
}

Только JSON, без дополнительного текста.`;

  const textContent = await callOpenRouter(prompt, 1024);

  const result = safeParseJSON(textContent) as { feathers: { stopCrane: string[]; mantra: string[]; ritual: string[] } };
...
5. ЗНАМЕНИТОСТИ с похожим типом (3-5 человек)
Мировые и российские знаменитости с ПОХОЖИМ типом личности.
Для каждого укажи:
- name: полное имя (Elon Musk, Павел Дуров, Steve Jobs)
- description: почему похож (1 предложение)
- imageQuery: запрос для поиска фото (например "Elon Musk portrait")

6. ХОББИ для души (5 занятий)
Чем этому человеку понравится заниматься, где он реализует сильные стороны?
...
Ответь СТРОГО в формате JSON:
{
  ...
  "celebrities": [
    {"name": "Elon Musk", "description": "Такая же комбинация упрямства и креативности", "imageQuery": "Elon Musk portrait"}
  ],
